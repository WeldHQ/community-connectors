...
//1 API URL
const baseSubscriptionsUrl = 'https://api.rechargeapps.com/subscriptions?limit=250';
...
exports.handler = async (req, res) => {
  ...
  //3.4 Call the API
  const resp = await axios.get(req.body.state?.nextUrl||baseSubscriptionsUrl, {
    headers: {
  ...
  switch (resp.data.next_cursor) {
    case null:
      result = {
        insert: resp.data.subscriptions,          // Data to be inserted into warehouse
        state: {nextUrl: baseSubscriptionsUrl},   // Define any variables here. Can be used for e.g. pagination or incremental pointers
    ...
    default:
      result = {
        insert: resp.data.subscriptions,                                                            // Data to be inserted into warehouse
        state: {nextUrl: baseSubscriptionsUrl.concat('&cursor=').concat(resp.data.next_cursor)},    // Define any variables here. Can be used for e.g. pagination or incremental pointers
...
// Schema for subscriptions table (null, string, int, long, float, double, boolean). If some fields are not defined, Weld will auto-infer types.
const getSchema = () => {
  return {
    schema: {
      subscriptions: {
