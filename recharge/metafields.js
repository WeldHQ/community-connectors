...
//1 API URL
const baseMetafieldsUrl = 'https://api.rechargeapps.com/metafields?limit=250';
...
exports.handler = async (req, res) => {
  ...
  //3.4 Call the API
  const resp = await axios.get(req.body.state?.nextUrl||baseMetafieldsUrl, {
    headers: {
  ...
  switch (resp.data.next_cursor) {
    case null:
      result = {
        insert: resp.data.metafields,          // Data to be inserted into warehouse
        state: {nextUrl: baseMetafieldsUrl},   // Define any variables here. Can be used for e.g. pagination or incremental pointers
    ...
    default:
      result = {
        insert: resp.data.metafields,                                                            // Data to be inserted into warehouse
        state: {nextUrl: baseMetafieldsUrl.concat('&cursor=').concat(resp.data.next_cursor)},    // Define any variables here. Can be used for e.g. pagination or incremental pointers
...
// Schema for metafields table (null, string, int, long, float, double, boolean). If some fields are not defined, Weld will auto-infer types.
const getSchema = () => {
  return {
    schema: {
      ...
      metafields: {
        ...
        items: {
          ...
          fields: [
            ...
            { name: 'owner_id', type: ['null', 'integer'], default: null },
            { name: 'owner_resource', type: ['null', 'string'], default: null },
