...
//1 API URL
const baseOnetimesUrl = 'https://api.rechargeapps.com/onetimes?limit=250';
...
exports.handler = async (req, res) => {
  ...
  //3.4 Call the API
  const resp = await axios.get(req.body.state?.nextUrl||baseOnetimesUrl, {
    headers: {
  ...
  switch (resp.data.next_cursor) {
    case null:
      result = {
        insert: resp.data.onetimes,          // Data to be inserted into warehouse
        state: {nextUrl: baseOnetimesUrl},   // Define any variables here. Can be used for e.g. pagination or incremental pointers
    ...
    default:
      result = {
        insert: resp.data.onetimes,                                                            // Data to be inserted into warehouse
        state: {nextUrl: baseOnetimesUrl.concat('&cursor=').concat(resp.data.next_cursor)},    // Define any variables here. Can be used for e.g. pagination or incremental pointers
...
// Schema for onetimes table (null, string, int, long, float, double, boolean). If some fields are not defined, Weld will auto-infer types.
const getSchema = () => {
  return {
    schema: {
      ...
      onetimes: {
        ...
        items: {
          ...
          fields: [
            ...
            { name: 'is_cancelled', type: ['null', 'boolean'], default: null },
            { name: 'next_charge_scheduled_at', type: ['null', 'string'], default: null },
